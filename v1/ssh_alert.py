...
def create_directory():
    ...
    # Check if the log directory exists, if not, attempt to create it
    if not os.path.exists(log_directory):
        try:
    ...
    # Check if the log file exists, if not create it with the required permissions
    if not os.path.exists(log_file_path):
        try:
...
def should_alert(user, ip):
    return all(u != user or i != ip for u, i, t in alert_log)
